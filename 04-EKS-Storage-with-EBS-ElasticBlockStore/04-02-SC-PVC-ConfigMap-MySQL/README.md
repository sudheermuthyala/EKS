...
- So if you don't add this `VolumeBindingMode` to WaitForFirstConsumer then it is going to immediately dynamically provision and then mount your volume.So means like it is going to provision your volume there.So in which availability zone we really don't know,it is dynamic, whappen So it can be in available zone one or means like US-East-1 here or US-East-1 we really don't know.
